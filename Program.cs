...
class Program
{
    ...
    private static MyAnimeListConfig malConfig { get; set; } = File.Exists(configFilePath) ? configDeserializer.Deserialize<MyAnimeListConfig>(File.ReadAllText(configFilePath)) ??  new MyAnimeListConfig() : new MyAnimeListConfig();


    private static void SaveConfig() {
        Directory.CreateDirectory(configDir);
        using var writer = File.CreateText(configFilePath);
        configSerializer.Serialize(writer, malConfig);
    }
    ...
    {
        malClient.SetClientId(malConfig.ClientId);
        malClient.SetAccessToken(malConfig.AccessToken);
        ...
        authCommand.SetHandler((ListProvider provider) =>
        {
            switch (provider)
            {
                case ListProvider.MyAnimeList:
                    malClient.SetClientId(malConfig.ClientId ??= getMalClientId());
                    malClient.SetAccessToken(malConfig.AccessToken = getMalAccessToken(malConfig.ClientId).Result);
                    SaveConfig();
                    break;
        ...
        .AddChoices(from search in (await Task.WhenAll(
        ...
        var rootCommand = new RootCommand(
            description: "Fire-and-forget app allowing users to sync their anime and manga lists between multiple providers")
        {
            authCommand,
            searchCommand
        };
    ...
    .StartAsync("Preparing HTTP Listener...", async ctx =>
    {
        ...
        var buffer = System.Text.Encoding.UTF8.GetBytes(@"<script>open(location, '_self').close();</script>");
